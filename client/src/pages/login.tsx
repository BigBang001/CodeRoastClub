...
export default function LoginPage() {
  ...
  const onSubmit = async () => {
    ...
    if (ok) {
      toast({ title: `Welcome back, ${username}!`, description: "Let’s see if your code improved… probably not 😏." });
      setLocation("/");
    }
  };

  const continueAsGuest = async () => {
    const res = await fetch('/api/auth/guest', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' } });
    if (res.ok) {
      const user = await res.json();
      toast({ title: `Guest mode enabled`, description: `You’re signed in as ${user.username}` });
      setLocation('/');
    }
  };
    ...
    <div className="min-h-screen bg-github-dark text-text-light">
      ...
      <main className="max-w-md mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <Card className="bg-[#1E222A] border-gray-700 text-gray-200 shadow-xl">
          <CardHeader>
            <CardTitle>Welcome back, Code Criminal 👨‍💻</CardTitle>
            <CardDescription className="text-gray-400">Sign in to save your roast history.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <Button disabled={loading} onClick={onSubmit} className="w-full bg-[#FF6B81] hover:bg-[#ff8ba0]">Sign in</Button>
            <div className="text-xs text-gray-500 text-center">or</div>
            <Button variant="outline" onClick={() => setLocation("/register")} className="w-full border-gray-600">Create an account</Button>
            <Button variant="ghost" onClick={continueAsGuest} className="w-full mt-2">Continue as Guest</Button>
          </CardContent>
        </Card>
      </main>
    </div>
